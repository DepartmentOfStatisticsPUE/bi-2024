...
for (r in 1:R) {
  S_A <- pop_data[which(rbinom(N, 1, s_A_sel) == 0),]         ## losujemy jednostki do próby S_A
...
for r in range(R):
    S_A = pop_data[np.random.binomial(1, s_A_sel, size=N) == 0]
...
for (r in 1:R) {
  ...
  S_A <- pop_data[which(rbinom(N, 1, s_A_sel) == 0),]
